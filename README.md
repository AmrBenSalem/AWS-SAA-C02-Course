...
Limited to 100 per account by default.
Object max = 5 TB.
...
Encryption and decryption use only one key.
...
- KMS keys are stored in KMS in an encrypted form themselves (when they must be used, they are decrypted in memory).
- AWS services, applications, and the user can all use them.
- Think of them as a container for the actual physical keys.
...
- Generated by KMS and `GenerateDataKey` operation.
- Used to encrypt data LARGER than 4KB in size.
- Linked to a specific KEY so KMS can tell that a specific DEK was generated with a specific KEY.

KMS does not store the DEK, once provided to a user or service, it is discarded.
KMS doesn't actually perform the encryption or decryption of data using the DEK or anything past generating them.
...
- Ciphertext Version - Encrypted version of the DEK.
  - This is encrypted by the KEY that generated it.
  - In the future it can be decrypted by KMS using the KEY assuming you have the permissions.
...
5. Wehn trying to decrypt the data, KMS is used to decrypt the DEK, and then DEK is used to decrypt the data.
...
All KMS support key rotation.

- AWS automatically rotates the keys every 1 year.
...
KMS can create an alias which is a shortcut to a particular key.
Aliases are also per region. You can create a `MyApp1` alias in all regions but they would be separate aliases, and in each region it would be pointing potentially at a different key.

#### 1.4.6.4. Key Policy (resource policy)

- Every KMS key has one.
- Customer managed keys can adjust the policy.
- Unlike other policies, KMS has to be explicitly told that keys trust the AWS account that they're in (using a resource policy with account arn as "Principal").
- An IAM policy is also needed for services or users to be able to use a KMS key.
...
Both types are encryption at rest.
Data sent from a user to S3 is automatically encrypted in transit outside of these methods.
...
- Server-side encryption is now mandatory in S3.
...
- S3 services manages the actual encryption and decryption
  - Offloads CPU requirements for encryption and decryption.
...
3. A hash (hash is one-way generated, cannot generate original text from hash) of the key is generated and attached to the object.
...
To decrypt the object, you must tell S3 which object to decrypt and provide it with the key used to encrypt it.
If the key that you supply is correct (with the proper hash), S3 will decrypt the object, discard the key, and return the plaintext version of the object.

#### 1.4.8.2. SSE-S3 AES256 (Server-side encryption w/ Amazon S3 managed keys)

AWS handles both the encryption and decryption process as well as the key generation and management.
This provides very little control over how the keys are used, but has little admin overhead.

SSE-S3 Encryption Steps

1. When putting data into S3, only need to provide plaintext.
2. S3 generates fully managed and rotated key automatically.
3. Object generates a key specific for each object that is uploaded.
4. The key is used to encrypt the specific object key, and the unencrypted version of that key is discarded.
...
KMS handles the keys and not S3.
The first time an object is uploaded, S3 works with KMS to create an AWS managed key.
This is the default key which gets used in the future.

Every time an object is uploaded, S3 uses a dedicated key to encrypt that object and that key is a data encryption key which KMS generates using the key generated above.
The key does not need to be managed by AWS and can be a customer managed key.

SSE-KMS Encryption Steps

1. S3 is provided (by KMS) a plaintext version of the data encryption key as well as an encrypted version.
2. The data is encrypted with the plaintext key and then it is discarded.
3. The encrypted key is stored alongside the encrypted object.

When uploading an object, you can create and use a customer managed keys.
This allows the user to control the permissions and the usage of the key material.
...
It also provides role separation, because you need a policy to access the KMS key to be able to encrypt or decrypt s3 objects.
(For administrators, a KMS deny must be in the IAM policy in order to disable their access to s3 objects.)


### 1.4.8,5 S3 Bucket keys

- Using SSE-KMS, every uploaded object will have a DEK (Data Encryption Key) generated and stored with it.
- This might be pricy, and there might be throttling issues (the generate data encryption key can only be run either 5500 10000 or 50000 per second (depending on the region), and this is shared across regions).

- What bucket keys help with : KMS generates a time limited key, that is given to S3.
- The key is periodically rotated by S3 (recreated using KMS key).
- Using that key, S3 will generated the DEKs.
- Reduces AWS KMS request costs by up to 99 percent by decreasing the request traffic from Amazon S3 to AWS KMS.
...
- Objects are replicated to at least 3+ AZs when they are uploaded. (Algorithms are used to detect and fix any data corruption in between AZs)
- 99999999999% durability
- 99.99% availability
- Offers low latency and high throughput.
- No minimums, delays, or penalties.
- Billing is storage fee, data transfer fee, and per 1000 requests based charge.

All of the other storage classes trade some of these compromises for another.

-> Use for frequently accessed data which is important and non replaceable.
...
- The storage fees are ~50% cheaper than S3 standard.
- Minimum capacity of 128KB charge for each object.
- 30 days minimum duration charge per object stored.
- Retrieval fee for every GB of data retrieved from this class.
- 3+ AZs

-> Use for long-lived data, which is important but has infrequent access.

#### 1.4.9.3. One Zone-IA

- Designed for less important data that is accessed less frequently but needed quickly.
...
-> Use for long-lived data, which is NON-CRITICAL and replaceable and has infrequent access.

#### 1.4.9.4. S3 Glacier-Instant

- Similar to S3 Standard-IA with cheaper storage but more expensive retrieval and longer minimum.
- More expensive retrieval per GB.
- Minimum duration charge of 90 days per object.
- Still have instant access to data.
- 3+AZs.

-> Use for long-lived data, which is important but accessed once per "quarter" with instance access.

#### 1.4.9.5. S3 Glacier-Flexible
...
-> Use for archival data with infrequent and no-realtime access (e.g. yearly access).

#### 1.4.9.6. S3 Glacier Deep Archive
...
-> Use for archival data that rarely (if ever) need to be accessed.

#### 1.4.9.7. S3 Intelligent-Tiering

- Combination of "almost" all above modes.
- Uses automation to remove overhead of moving objects.
- Additional fee per 1,000 objects tracked.
- There are NO retrieval costs.

- Objects starts at Standard.
- If an object is not accessed for 30 days, it will move into Standard-IA.
- And eventually to other layers.
- Layers with long access time are optional.

-> Use for long-lived data with unknown their access pattern.
...
Change the storage class over time such as (example):
...
With the exception of S3 one zone-IA not able to transition into S3 glacier instant retrieval

- Transition of smaller objects can cost more (because of minimum size pricing)
- Minimum of 30 days before transition from S3 Standard in lifecycle configuration.
- If above transition happened to Standard-IA or One Zone-IA, another wait of 30 days must occur before transition to Glacier.
...
- Replication is not retroactive.
  - If you enable replication on a bucket that already has objects, the old objects will not be replicated. (can be configured to do so)
- Both buckets must have versioning enabled.
- It is a one way replication process only (if objects are added to the destination, they will not be replicated to the source).
- Replication by default can handle objects that are unencrypted or SSE-S3 and SSE-C.
...
- Will not replicate system events (changes by lifecycle management, only used events are replicated), glacier, or glacier deep archive.
- No deletes are replicated by default (but can be enabled).

#### 1.4.11.3. Why use replication

SRR - Log Aggregation (from multiple buckets to a single bucket)
SRR - Sync production and test accounts
SRR - Resilience with strict sovereignty requirements (for comapnies that have strict data policies)
CRR - Global resilience improvements
CRR - Latency reduction (replicate data to a region closer to customers)
...
- Don't generate presigned URLs with an IAM role (probably a lambda that assumes a role to generate presigned ?), use IAM users instead.
...
### 1.4.14. S3 event notification

- When enabled, notifications are generetad when events occur in buckets.
- Can be delivered to SQS, SNS and lambda functions (destinations need resource policies to allow S3 to interact with them).
- Events can be :
  - Objects created
  - Objects deleted
  - Objects restored
  - Objects replicated
- EventBridge can be used instead, which offers more types of events and more services.
- EventBridge should be the default go-to.

### 1.4.15. S3 access logs

- Logs bucket and object access to a target bucket.
- Access logging can be enabled via UI or PUT command.
- Target bucket ACL should allow "S3 log delivery group".

### 1.4.16. S3 object lock

- Object lock can be enabled on "new" buckets. (for old buckets, AWS support must be contacted)
- Versioning is also enabled when enabling object lock.
- You cannot disable object lock or suspend versioning if object lock is active.
- Object lock is following the WORM (write-once-read-many) -> No Delete and no overwrite.
- Versions are locked (no changes for current versions).
- Objects can be configured with retention period and legal hold.
  - They can have one of them, both of them, or none of them.
- Buckets can have a default object lock settings.
- Prevent accidental deletion of critical object versions.

#### 1.4.16.1 Retention period

- Specify retention period in days or years.
- Different modes for setting the period :
  - Compliance : Object version (and retention period itself) can't be adjusted, deleted or overritten during that period.
  - Governance : special permissions can be granted allowing lock settings to be changed.

#### 1.4.16.2 Legal hold

- Set an object version to ON or OFF, no deletion or changes until OFF.
- No retention.
- Special permission is required to add or remove.

### 1.4.17 Access points

- Simplifies managing access to S3 buckets/objects.
- We can imagine the case of a bucket with millions of objects.
- Rather than 1 bucket with 1 very complex bucket policy.
  - We create multiple access points with a different policy and different network access control each.
- Each access point has its own endpoint address.
- Created via :
  - Console UI
  - OR aws s3control create-access-point
- Access points can be configured for access via VPC, requires a VPC endpoint.
- Any permission granted on the access point policies must be granted on the bucket policy.
  - What can be done, broad access to "everything" in bucket policy, and more granular permissions attached to access point policies.

### 1.4.17.1 Multi region Access points

- Access points on a set of buckets in multiple regions.
- Buckets can be set to be active or passive (passive will be active when no other active buckets).
- All buckets can be active (customer access the closest)
- Replication can be set up
  - One way replication : one directional replication from one bucket -> other buckets.
  - Multi way replication : from all buckets to all buckets (sync buckets)
- The multi region access points comes with an ARN (and probably a DNS link) that can be used as a single entry to the AP.
  - It redirects to the cloest location that has an S3 bucket.
    - Can raise issues (consistency lag), if we're trying to access a file that the closest bucket didn't yet replicate from the other buckets.
---
